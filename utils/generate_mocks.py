...
def main():
    ...
    # Flow
    flow = create_flow(features=1, context_features=16, num_layers=8, hidden_features=32).to(device)
    save_mock_checkpoint(flow, CHECKPOINT_DIR / "flow_best.pt")

    # Diffusion
    diff = DenoisingMLP(embed_dim=EMBEDDING_DIM, cond_dim=32, hidden_dim=512).to(device)
